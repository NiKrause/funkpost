...
/**
 * The firmware measures its own transmit airtime and refuses to exceed the
 * regional duty cycle — `DUTY_CYCLE_LIMIT`. That is neither "probably
 * delivered" nor "broken": the frame definitely did not go out, and it
 * definitely can later. Marked so the courier can wait instead of giving up,
 * and so it learns that its own budget estimate was too generous — the radio
 * spends airtime on beacons and relaying that no local model can see.
 */
export const DUTY_CYCLE_LIMIT = 9;
...
export function createMeshtasticDeviceLink({
  ...
  // How many of OUR frames the radio gave up retransmitting. See the swallow
  // site below for why this cannot be read off the console instead.
  let softRefusals = 0;
  let lastSoftRefusal = null;
  ...
  return {
    ...
    /**
     * Frames of ours the radio stopped retransmitting, and the last reason.
     *
     * `MAX_RETRANSMIT` on a broadcast means the node waited for an implicit ack
     * — a neighbour rebroadcasting — and never heard one, after spending its
     * retries. On a bench where nothing relays, that is the normal outcome and
     * the cost is real: issue #73 asks how large it is, and this is the number
     * that answers it.
     */
    get refusals() {
      return { soft: softRefusals, last: lastSoftRefusal };
    },
    ...
    async send(bytes) {
      ...
      try {
        ...
        //
        // Counted, though, because swallowing is not the same as free. Reaching
        // here means the radio ran out its own retransmissions for a frame of
        // ours, and every one of those retries was airtime. This is the only
        // place that can attribute such a refusal to *us*: the client library
        // logs one for every packet the node sends, its own telemetry
        // included, so a console count says nothing about our share.
        if (typeof refusal?.error === "number" && SOFT_ROUTING_ERRORS.has(refusal.error)) {
          softRefusals++;
          lastSoftRefusal = routingErrorName(refusal.error);
          return;
        }
        // The radio hit the airtime law. Not a failure — a "not yet".
        if (refusal?.error === DUTY_CYCLE_LIMIT) {
          const wait = new Error("the node has spent its hourly airtime — waiting");
          wait.dutyCycleExhausted = true;
          throw wait;
        }
...
/**
 * Report the node's modem preset — `"LONG_FAST"`, `"SHORT_TURBO"`, … — from the
 * same config packet the region arrives in.
 *
 * It matters more than it looks. The preset sets the data rate, and the spread
 * between them is a factor of twenty: LONG_FAST moves 1 070 bit/s, SHORT_TURBO
 * 21 880. A courier pacing to the wrong one is wrong by that factor — waiting
 * twenty times longer than the airtime law requires, and reporting a budget
 * that bears no relation to what the radio is actually spending. On a bench
 * where SHORT_TURBO is the sensible choice, assuming LONG_FAST makes every
 * measurement useless.
 *
 * @param {Object} device A MeshDevice
 * @param {(preset: string) => void} cb
 * @returns {() => void} unsubscribe
 */
export function watchModemPreset(device, cb) {
  return subscribe(device.events.onConfigPacket, (config) => {
    const variant = config?.payloadVariant;
    if (!variant || variant.case !== "lora") return;
    const code = variant.value?.modemPreset ?? 0;
    const name = Protobuf.Config.Config_LoRaConfig_ModemPreset[code];
    if (typeof name === "string") cb(name);
  });
}
...
/**
 * Report **every** mesh packet the node hands over, decodable or not.
 *
 * This exists because of the quietest failure in the stack. A packet the node
 * cannot decrypt — the peer transmitting on a channel whose key this node does
 * not hold — is dropped inside the client library with
 * `log.debug("🔐 Device received encrypted data packet, ignoring")` and nothing
 * else. It never reaches a port, so `onPrivatePacket` never fires and the app's
 * received-frame count stays at zero, exactly as if nobody were there at all.
 *
 * Two very different situations, one indistinguishable symptom: *nobody is in
 * range* and *somebody is in range and we cannot read a word they say*. This
 * event separates them, because it fires before the decrypt decision.
 *
 * @param {Object} device A MeshDevice
 * @param {(packet: Object) => void} cb Receives the raw `MeshPacket`; its
 *   `payloadVariant.case` is `"decoded"` or `"encrypted"`
 * @returns {() => void} unsubscribe
 */
export function watchMeshTraffic(device, cb) {
  return subscribe(device.events.onMeshPacket, cb);
}
