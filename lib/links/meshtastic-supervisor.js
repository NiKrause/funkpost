 ...
 * - **A configure that never finishes is the one state nothing rescued.**
 *   `configure()` is started and not awaited, because the watchers above it
 *   are what carry the answers. But when its stream stalls — a lost packet, a
 *   node still booting after a channel import — nothing arrives, nothing
 *   fails, and no timer fires: backoff and give-up only watch *disconnects*.
 *   The link then looks connected while the courier stays at region `UNSET`
 *   and refuses every transmission, which from the outside is
 *   indistinguishable from a radio that cannot hear anyone. So there is a
 *   deadline now, and missing it says what did not arrive.
 ...
 * @param {number} [options.reattachGapMs] How long a repair must hold before
 *   another one is allowed. Measured in the field: without this, a repair that
 *   fails immediately produces the next one at once, dozens per second.
 * @param {number} [options.configureTimeoutMs] How long the node has to send
 *   its configuration before `configureTimeout` fires (default 20 000). The
 *   config stream normally completes in about a second.
...
export async function connectMeshtasticDevice({
  ...
  /** Callers injected timers before this needed a clock, so tolerate its absence. */
  ...
  let seen = { region: false, myNodeInfo: false };
   ...
   */
  /**
   * The node has answered. One of these is enough to say the stream is
   * running: a stall shows as silence, not as half a configuration.
   */
  const noteConfigured = (what) => {
    seen = { ...seen, [what]: true };
    if (configured) return;
    configured = true;
    if (configureTimer) {
      timers.clearTimeout(configureTimer);
      configureTimer = null;
    }
  };
  ...
  const attach = (dev) => {
    ...
    unsubscribes.push(
      ...
      watchModemPreset(
        dev,
        mine((name) => {
          if (courier?.setPreset) courier.setPreset(name);
          fire("preset", name);
        }),
      ),
      watchAirUtilTx(
      ...
      watchMyNodeInfo(
        dev,
        mine((info) => {
          noteConfigured("myNodeInfo");
          fire("myNodeInfo", info);
        }),
      ),
      ...
      watchDeviceStatus(
        ...
        mine((name) => {
          ...
          //
          // But a repair that did not hold is not ground truth about anything.
          // Measured in the field on Android Chrome: reattach → configuring →
          // connected → disconnected within milliseconds, then again, dozens
          // of times a second, every cycle logging "GATT operation already in
          // progress" — the previous operation had not finished, and asking
          // the stack for another one is what broke it. When the last repair
          // is younger than the gap, the link is not merely reporting badly;
          // it needs the real thing, which backs off and waits.
          const repairedRecently =
            lastReattachAt != null && now() - lastReattachAt < reattachGapMs;
          if (isLinkAlive && isLinkAlive() && reattaches < maxReattaches && !repairedRecently) {
            void reattach();
          } else void reconnect();
        }),
      ),
    ...
    // The deadline: `myNodeInfo` is the node saying who it is, and the region
    // is what the courier needs before it will transmit. Either one arriving
    // means the stream is running; neither arriving is the stall this catches.
    if (configureTimer) timers.clearTimeout(configureTimer);
    configured = false;
    configureTimer = timers.setTimeout(() => {
      configureTimer = null;
      if (configured || closed || dev !== currentDevice) return;
      fire("configureTimeout", {
        afterMs: configureTimeoutMs,
        missing: seen.region ? ["myNodeInfo"] : seen.myNodeInfo ? ["region"] : ["region", "myNodeInfo"],
      });
    }, configureTimeoutMs);
  ...
  /**
   * Count the link healthy once it has *stayed* up — and only then.
   *
   * Both budgets come back, because both were spent on a problem that is over.
   * `reattaches` in particular must: Android Chrome reports a failed GATT write
   * as a disconnection and does so readily, so a cap that only ever counts up
   * is exhausted within minutes on a link that never went away. After that
   * every write error takes the reconnect path and the app reports a dropped
   * link while the connection is still up — measured on a phone in the field,
   * which complained and kept working at the same time.
   *
   * A connect that drops again at once never reaches this, so a flapping stack
   * still climbs toward the caps instead of looking healthy after every try.
   */
  const markHealthyIn = (ms) => {
    if (stableTimer) timers.clearTimeout(stableTimer);
