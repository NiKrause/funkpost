...
export function createTokenBucket({ dutyCycle, windowMs = 3_600_000, now = Date.now } = {}) {
  ...
  return {
    ...
    /**
     * How long until `costMs` of airtime could be spent — **without spending
     * it**. Zero when it is affordable now.
     *
     * Separate from `take` because a screen needs to say "in four minutes"
     * without reserving anything to find out, and because the bucket can be in
     * debt: `remainingMs` clamps at zero, so it cannot answer this on its own.
     */
    delayFor(costMs) {
      refill();
      const after = tokensMs - costMs;
      return after >= 0 ? 0 : -after / dutyCycle;
    },
    ...
    /**
     * Overwrite the remaining budget with an externally measured truth —
     * the node's own airtime accounting. Clamped to [-capacity, capacity]:
     * a node that overspent (relaying counts too) pushes the bucket into
     * debt and pacing waits for the refill to catch up.
     */
    setRemainingMs(ms) {
      refill();
      tokensMs = Math.min(capacityMs, Math.max(-capacityMs, ms));
    },
  };
}
