 ...
 *
 * Implements the seam contract from @le-space/orbitdb-storage-bridge/courier-sync
...
export function createMeshtasticCourier({
  ...
  return {
    ...
    /**
     * How long until one frame of `byteLength` could go out, in milliseconds.
     * Zero when there is budget now, and zero in regions without a duty cycle.
     *
     * For a screen to say *when* rather than only *not now* — and asking must
     * not itself spend anything, which is why it does not go through `take`.
     */
    timeUntilAffordable(byteLength = link.mtu) {
      if (!bucket.delayFor) return 0;
      return bucket.delayFor(estimateAirtimeMs(byteLength, modemPreset));
    },
