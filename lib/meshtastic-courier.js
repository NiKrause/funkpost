...
export function createMeshtasticCourier({
  ...
  const policy = typeof region === "object" && region !== null
    ? { region: region.region || "custom", dutyCycle: region.dutyCycle ?? null, misconfigured: false }
    : regionPolicy(region);
  ...
  const defaultRto = (fragments) =>
    2 * (fragments + 1) * estimateAirtimeMs(link.mtu, preset) + 500;
  const defaultGap = () => 3 * estimateAirtimeMs(link.mtu, preset) + 250;

  /** Pace one frame through the airtime budget, then hand it to the link. */
  const transmitFrame = async (frame) => {
    const airtimeMs = estimateAirtimeMs(frame.length, preset);
    const waitMs = bucket.take(airtimeMs);
  ...
  const finishSend = (msgId, state, error) => {
    ...
    if (error) {
      emit({ kind: "giveup", msgId, rounds: state.rounds });
      state.reject(error);
    } else {
  ...
  const transmitWave = async (msgId, state) => {
    ...
    if (state.rounds > maxRounds) {
      finishSend(msgId, state, new Error(`message ${msgId} undelivered after ${maxRounds} rounds`));
      return;
    }
    ...
    try {
      for (const idx of [...state.outstanding]) {
        ...
        await transmitFrame(state.frames[idx]);
      }
    } finally {
      state.sending = false;
    }
  ...
  return {
    /**
     * Deliver one payload to the peer. Resolves when the receiver has
     * acknowledged every fragment; rejects after `maxRounds` transmission
     * rounds, or immediately when the node's region is not configured.
     */
    async send(payload) {
