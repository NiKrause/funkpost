...
const name = isDefault ? Buffer.alloc(0) : Buffer.from(opt("name", "bench"), "utf8");
if (!isDefault && (name.length === 0 || name.length > 11)) {
...
if (isDefault) {
  ...
  if (!isPublicShorthand && ![16, 32].includes(psk.length)) {
    console.error(`✗ a key is 16 or 32 bytes, or \`01\` for the public one; got ${psk.length}`);
    process.exit(1);
  }
} else {
...
const settings = isDefault
  ? lenDelim(2, psk)
  : Buffer.concat([lenDelim(2, psk), lenDelim(3, name), fixed32(4, id)]);
...
try {
  ...
  if (isDefault) {
    ...
    // sharing the public key — which is not what was lost.
...
console.log(`  ${url}`);
console.log();
console.log(`  name          ${isDefault ? "(none — the preset supplies “LongFast”)" : name}`);
...
if (isDefault) {
  console.log(`  key           the well-known public one — everybody can read this channel`);
} else {
